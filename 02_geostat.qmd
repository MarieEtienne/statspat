...
On cherche donc à associer à chaque $h$ $\E( (Z(s+h) - Z(s))^2 )$, on va approcher cette espérance par $\left (Z(s_i)- Z(s_j) \right^2$
...
<!-- $$\widetilde \gamma (d_k) = \frac{1}{B(d_k)}\left\{\mbox{med}(|Z(s_i)-Z(s_j)|)^{1/2},(i,j) -->
<!--      \in C(k)\right\}^{4}\; B(d_k) \to 0.457$$ -->
     

## Ajustement  du variogramme

$\gamma_{\rho,C,\nu,\tau}$} variogramme admissible, \coul{$(\rho,\nu, C, \tau)$} solution du problème
   $$\min_{\rho,C,\nu,\tau} \sum_{k=1}^K (\gamma_{\rho,C,\nu,\tau}(d_k) - \widetilde \gamma(d_k))^2$$
Autres m\'ethodes :\\
max de vraisemblance, moindres carrés généralisés

# Prédiction : le krigeage

## Objectifs 

   On observe \coul{$z(s_1),z(s_2),\ldots,z(s_n)$} aux sites $s_1,s_2,\ldots,s_n$
...
Prédire $Z(s_0)$ sachant $Z(s_1)=z(s_1),\ldots,Z(s_n)=z(s_n)$
   ...
   $$p^{opt} = \mbox{E}\big(Z(s_0)/Z(s_1)=z(s_1),\ldots,Z(s_n)=z(s_n)\big)$$
