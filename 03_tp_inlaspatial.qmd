---
title: "TD5 - Ajuster un modèle hiérarchique spatial avec INLA"
author:
...
Nous nous intéressons à un jeu de données concernant la prévalence de la
malaria en Gambie (disponible dans le package {geoR}). Cet exemple est
repris du livre "Spatial and Spatio-temporal Bayesian Models with
R-INLA".
...
-   l'administration éventuel d'un traitement sur la moustiquaire
    `treated`,
-   une mesure par satellite de la végétation proche `green`,
-   la présence ou non d'un centre de santé dans le village.

1.  Calculer la prévalence (proportion de cas positif) dans chaque
    village
...
2.  Indiquer ce que signifie à votre avis `crs = "+proj=utm +zone=28"`
...
Dans les approches INLA, le champs gaussien caché est vu comme la
solution d'une Equation aux Dérivées Partielles Stochastiques (EDPS) qui
va être résolue sur un maillage. On peut définir le maillage.
...
Mais pour résoudre l'EDPS, on a besoin de mettre des conditions de
nullité au bord du domaine ce qui a des conséquences forte sur la
solution. Pour limiter l'impact on ajoute buffer autour avec une maille
plus large.
...
4.  Il est naturel d'espèrer un résultat plus fin aux endroits où l'on a
    des données, on peut lui passer les localisations du jeu de données
    pour construire le maillage grâce à l'argument `loc`. Que constatez
    vous ?
...
On définit à partir du maillage le champ spatial SPDE qui correspond à
un champ spatial gaussien avec une covariance Matérn. Nous considérons
les lois a priori par défaut sur les paramètres de variance et de portée
du champ spatial.
...
L'argument `constr = TRUE` impose une moyenne nulle pour le champs
spatial.

On souhaite ajuster le modèle suivant

Pour l'enfant $j$ du village $i$, on suppose que conditionnellement aux
covariables et à un champs spatial caché, l'infection à la malaria est
le résultat d'un tirage de bernoulli.
...
La mise en oeuvre dans INLA est ensuite un peu technique (le package
inlabru permettrait de simplifier les étapes suivantes)
...
6.b Une matrice de projection. Cette matrice fait le lien entre les
positions des observations et les noeuds du maillage. C'est une matrice
très creuse.
...
6.c On doit ensuite empiler ces données dans une structure spécifique à
INLA
...
7.  On peut appeler INLA. Explorer les sorties contenues dans l'objet
    `fit`
...
8.  Accéder aux distributions marginales des effets aléatoires et des
    hyperparamètres :
...
9.  Tacer les distributions a posteriori marginales des effets, par
    exemple pour l'effet de l'âge ou de l'utilisation de la moustiquaire
    :
...
10. Représenter la composante spatiale de la malaria qui n'est pas
    expliquée par les covariable. Pour ceci, on peut créer un jeu de
...
11. le package `geodata` contient les altitudes en Gambie. Reprenez les
    étapes pour ajuster un modèle dans lequel on explique la prévalence
    de la malaria par l'altitude. On pourra alors grouper les données
    par village.
